import './description.css';


const Description = () => {
    return <div className="description_container">
        <div className="description_header">
            الوصف
        </div>
        <div className="body">
            درس تصميم واجهات المستخدم (UI Design) هو رحلة متكاملة لتعلّم كيفية تحويل
            الأفكار إلى تصاميم جذابة وسهلة الاستخدام. يبدأ بشرح أساسيات التصميم،
            الألوان، الخطوط، وتناسق العناصر على الشاشة.تعرّف الطالب على مبادئ
            التصميم الجيد وكيفية بناء تجربة بصرية مريحة وواضحة.يشمل الكورس تطبيقات
            عملية باستخدام أدوات احترافية مثل Figma وAdobe XD.سيتعلم الطالب كيفية
            تصميم صفحات لتطبيقات الموبايل والمواقع بشكل احترافي. يتم شرح الأخطاء
            الشائعةفي التصميم وكيفية تجنبها بأسلوب بسيط ومباشر لتطبيقات الموبايل
            والمواقع احتراف في نهاية الكورس، ينفّذ الطالب مشروع تصميم حقيقي ليضيفه
            إلى ملفه الشخصي (Portfolio). يحصل المتدرّب علىتقييم وملاحظات لتحسين
            مستواه وتطوير مهاراته أكثر.
        </div>
        <div className="button_container">
            <button className="show_more_button">اظهار المزيد</button>
        </div>
    </div>
};
export default Description;